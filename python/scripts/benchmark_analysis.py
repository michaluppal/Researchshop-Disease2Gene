...
def compute_metrics(paper: dict, summary: dict) -> dict:
    """Compute P/R/F1 + 95% Wilson CIs for one paper."""
    gold_genes = {g.upper() for g in paper.get("expected_genes", [])}
...
def main():
    ...
    # Write CSV
    csv_columns = [
        "pmid", "type", "gold_gene_count", "extracted_gene_count", "true_positives",
        "precision", "recall", "f1",
    ...
    print(f"{'PMID':<12} {'Type':<20} {'Gold':>5} {'Extr':>5} {'TP':>4} "
          f"{'P':>6} {'R':>6} {'F1':>6} {'F1 CI 95%':>14} {'Cit':>6} {'Jac':>6}")
    print("-" * 92)
    ...
    print(f"{'Type':<20} {'n':>4} {'Macro F1':>10} {'Weighted F1':>13} {'Mean CI':>18}")
    print("-" * 70)
    for ptype, type_rows in sorted(by_type.items()):
        n = len(type_rows)
        ...
        print(f"{ptype:<20} {n:>4} {macro_f1:>10.3f} {weighted_f1:>13.3f} {ci_str:>18}")
