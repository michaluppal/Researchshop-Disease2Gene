...
SAMPLE_PAPERS = {
    "11111111": {
        ...
        "abstract": (
            ...
            "in 35% of tumors. Genetic variants were identified by genotype analysis."
        ),
        "PMID": "11111111",
    },
    "22222222": {
        ...
        "abstract": (
            ...
            "lung cancer patients using genomic sequencing. Exon 19 deletions "
            "(p.E746_A750del) and the L858R point mutation were the most common "
            "EGFR alterations. ALK gene rearrangements were detected in 5% of "
            "cases. Response to erlotinib was significantly higher in EGFR-mutant "
            "patients (p=0.0001). Variant allele frequency was measured."
        ),
        "PMID": "22222222",
    },
    "33333333": {
        "title": "TCF7L2 and PPARG gene variants in type 2 diabetes GWAS",
        "authors": ["Kim E", "Park F", "Chen G"],
        "year": "2024",
        "journal": "Cell",
        "affiliations": ["Stanford University", "Broad Institute"],
        "abstract": (
            "A GWAS meta-analysis of 100,000 individuals with genomic sequencing "
            "identified TCF7L2 rs7903146 as the strongest genetic risk locus for "
            "type 2 diabetes (OR=1.4, p=2.1e-50). Additional SNP signals at PPARG "
            "(Pro12Ala polymorphism), KCNJ11 (E23K variant), and SLC30A8 "
            "(Arg325Trp mutation) were confirmed by genotype analysis. "
            "Polygenic risk scores incorporating these allele variants improved "
...
def _gemini_stream_side_effect(model, contents, config=None):
    """Side effect for generate_content_stream that returns mock responses."""
    ...
    for pmid, full_text_data in SAMPLE_FULL_TEXT.items():
        ...
        if paper_content[:60] in prompt_text:
            target_pmid = pmid
            break

    if target_pmid is None:
        # Check abstracts for abstract-only calls
        for pmid, paper_info in SAMPLE_PAPERS.items():
            if paper_info["abstract"][:50] in prompt_text:
                target_pmid = pmid
                break
    ...
    if "extract the requested information" in prompt_text.lower():
        ...
        return _make_stream_chunks(json.dumps(response_data))
    else:
...
class InProcessRunner:
    """Replaces multiprocessing.Process to run target in a thread (same process).

    This allows unittest.mock patches to be visible inside the worker.
    """
    ...
    def join(self, timeout=None):
        if self._thread:
            ...
            self._alive = self._thread.is_alive()

    def is_alive(self):
        return self._alive

    def terminate(self):
        pass  # Cannot terminate a thread, but test should complete quickly
...
class TestPipelineIntegration:
    ...
    def setup_env(self, output_dir, tmp_path):
    ...
    def _run_pipeline(self):
        ...
        patches = [
            ...
            # Replace multiprocessing.Process with in-process thread runner
            # so our mocks are visible inside the worker
            patch(
                "modules.pipeline_orchestrator.mp.Process",
                InProcessRunner,
            ),
            # Mock the Gemini client constructor globally
            ...
            patch(
                "modules.gemini_extractor.get_rate_limiter",
                return_value=MagicMock(wait_if_needed=MagicMock()),
            ),
        ]
        ...
        try:
            # Force re-import to pick up the mp.Process patch
            import importlib
            import modules.pipeline_orchestrator as po
            importlib.reload(po)

            result = po.run_complete_pipeline(
    ...
    def test_gene_symbols_are_valid(self):
        ...
        # HGNC gene symbols: 1-15 uppercase letters/digits, may contain hyphens
        gene_pattern = re.compile(r"^[A-Z][A-Z0-9\-]{0,15}$")
    ...
    def test_variant_format(self):
        ...
        variant_patterns = [
            ...
            r"^p\.[A-Z]",           # protein: p.E746_A750del
    ...
    def test_column_order(self):
        ...
        assert cols[0] == "Gene/Group", (
            f"First column is '{cols[0]}', expected 'Gene/Group'"
        )
    ...
    def test_multiple_genes_per_paper(self):
        """Papers with multiple genes should produce multiple rows."""
        result = self._run_pipeline()
        df = pd.read_csv(result["local_path"])

        # Paper 11111111 has BRCA1, BRCA2, TP53 -> should have multiple rows
        paper1_rows = df[df["PMID"].astype(str) == "11111111"]
        genes_in_paper1 = set(
            paper1_rows["Gene/Group"].dropna().str.upper().unique()
        )
        assert len(genes_in_paper1) >= 2, (
            f"Paper 11111111 should have multiple genes, found: {genes_in_paper1}"
        )

    def test_abstract_column_present(self):
        """Each row should have an Abstract column with content."""
        result = self._run_pipeline()
        df = pd.read_csv(result["local_path"])
        assert "Abstract" in df.columns, "Missing 'Abstract' column"
        non_empty_abstracts = df["Abstract"].fillna("").str.len() > 20
        assert non_empty_abstracts.any(), "All abstracts are empty"
