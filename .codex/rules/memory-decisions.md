...
> Codex migration note (2026-04-25): append new decisions here for Codex sessions. Historical
> Claude-era entries are preserved for continuity.

> Append new decisions with date. Never delete entries — they explain why code is the way it is.

---

## 2026-04-25 — Codex becomes the active agent documentation system

**Decision:** Use `AGENTS.md` and `.codex/` as the active routing, memory, task recipe, and hook-note surface. Keep `archive/claude/` as legacy context instead of deleting the Claude-era files.
**Rationale:** The project is now maintained through Codex, but the Claude-era memory files contain useful historical context. A parallel Codex-native surface gives future sessions clear active instructions without losing audit trail.
**Files:** `AGENTS.md`, `.codex/rules/`, `.codex/tasks/`, `.codex/hooks/README.md`, `archive/claude/`
...
**Rationale:** The agent rules forbid guessing model names because stale IDs fail at runtime. The model was checked after review and confirmed valid/stable, so the current hardcoded query-builder endpoint is acceptable until it is next intentionally revised.
...
**Files:** `archive/claude/commands/annotate-paper.md` (legacy path), `python/scripts/extract_pmc_figures.js`
...
### `docs/audit/AUDIT.md` as quality source of truth
**Decision:** All pipeline bugs, fixes, and known tradeoffs are documented in `docs/audit/AUDIT.md`.
**Protocol:** Update `docs/audit/AUDIT.md` synchronously whenever pipeline modules change. Never let it drift.
