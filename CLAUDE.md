...
- Do not use `npm run package:mac:local` or `electron-builder --mac --config` for local DMG builds on Apple Silicon — the `electron-builder.yml` config targets `universal` arch which builds x86_64 first and hangs indefinitely at the packaging step on M1/M2/M3. Use `npm run package:mac:local` (now fixed to `--arm64 -c.mac.target=dmg`) or explicitly pass `--arm64 -c.mac.target=dmg` to skip the x86_64 cross-build. The `package:mac` script still targets universal for release distribution. (Caught: 2026-04-07)
