...
This is the W1 fix from [`docs/AUDIT.md`](docs/AUDIT.md). Pre-W1, step 3 ran without
...
`alpha-globin`-cleaned text and misses. That's flagged as **F6** in `Final_Audit.md`.

### 4.5 What is *not* in the fetcher

For context on what "scraping-free" means here — these were removed in F5 of
[`docs/AUDIT.md`](docs/AUDIT.md) and the module header documents it:
...
- **Paywall detection** — gone. The assumption is the OA filter makes paywalled papers
  impossible (see F2 in `Final_Audit.md` for why that assumption leaks).
...
- **No publisher HTML** — the article's landing page on Nature/Elsevier/Wiley/etc. is never
  requested. Removed in F5 of `AUDIT.md`.
...
This is the redundancy flagged in [F4](../Final_Audit.md#f4--redundant-fetches-across-the-uipipeline-boundary).
...
- **`thinking_config=ThinkingConfig(thinking_budget=0)`** — this *must* be set. Gemini
  ...
  prompts for 600+ seconds. This is [C20 in AUDIT.md](docs/AUDIT.md) and the memory
...
This is C9 in `AUDIT.md` — removed alias matching from the deterministic extractor
...
symbol" problem. The full story from `AUDIT.md` and `memory-decisions.md`:
...
`FINAL_VALIDATION_MIN_CONFIDENCE = 0.7` is marked `IMPORTANT` in `CLAUDE.md`:
...
This dict is printed to stdout as `RESULT:{json}` (see the IPC protocol in CLAUDE.md).
...
- **Specific bug history per stage** → `docs/AUDIT.md`. Each bug has a letter code
  (F1, F2, C19, C22, W10) and is referenced from relevant sections of this doc.
- **Stage-level overview for Claude sessions** → `.claude/rules/memory-pipeline.md`.
...
- **Architectural decisions with dates** → `.claude/rules/memory-decisions.md`.
  Why the pipeline is the way it is.
- **Current findings from end-to-end trace** → `Final_Audit.md` (F1–F10).
