...
**Why this is not a bug:** `AGENTS.md` explicitly marks `FINAL_VALIDATION_MIN_CONFIDENCE=0.7`
...
`docs/audit/AUDIT.md` (C22 sprint) can be reduced or eliminated by a structural change.
...
Documented in the C22 citation sprint ([`AUDIT.md`](AUDIT.md), 2026-02-25):
...
- **Output row count.** Economy mode sometimes collapses multiple variant rows of the
  ...
  a visible diff in benchmark outputs — flag in `docs/audit/AUDIT.md` when the mode is introduced.
...
The W1 fix in `docs/audit/AUDIT.md` made Greek letters work correctly in paper text by transliterating
...
From docs/audit/final-audit.md F10b entry as originally written (now corrected via the
...
- `vitest` ^4.1.4 devDependency, `config/vitest.config.ts`, `npm test` →
  `vitest run`
...
- `npx tsc --noEmit -p config/tsconfig.node.json` and `config/tsconfig.web.json` → only
...
- [`pipeline/modules/full_text_fetcher.py:1–14`](pipeline/modules/full_text_fetcher.py) opens
  ...
  publisher-specific scrapers were all removed as "unreachable dead code" (F5 in `docs/audit/AUDIT.md`).
...
2. **Silent quality degradation.** A researcher pasting a list of key papers for a review
   ...
   "silent failure at screening" failure mode flagged as unacceptable in `AGENTS.md`.
...
- [`archive/claude/rules/memory-decisions.md:164`](archive/claude/rules/memory-decisions.md:164) —
  "mitigated by overfetch factor (4x)"
- [`docs/pipeline/internals.md:369`](../pipeline/internals.md) —
  "`ANALYSIS_OVERFETCH_FACTOR=4` … fetches and analyzes 40 candidates"
- [`docs/pipeline/internals.md:1122`](../pipeline/internals.md) — config table
- [`docs/pipeline/reports/pipeline-report.tex:319`](../pipeline/reports/pipeline-report.tex)
...
- [ ] Remove/correct references in `memory-decisions.md`, `docs/pipeline/internals.md`,
      `docs/pipeline/reports/pipeline-report.tex`, `publication/working/*`.
...
- `archive/claude/rules/memory-decisions.md:164` — OA-gap mitigation corrected
- `archive/claude/rules/memory-pipeline.md:292` — replaced "overfetch factor" section with
...
- `grep ANALYSIS_OVERFETCH_FACTOR` returns only 3 intentional refs: 2 explanatory
  ...
  and docs/audit/final-audit.md's F1 finding record
...
- `docs/audit/final-audit.md` (this file) — the F1 finding IS the record that the claim was
...
- `archive/claude/rules/memory-sessions.md` — historical session log; do not rewrite
  history.
- `docs/pipeline/reports/pipeline-report.tex` — legacy PDF artefact, regenerate on next
  pandoc build.
...
## Session handoff notes (for future agent sessions)
...
- `9c2aecd` docs: mark F3 DONE in docs/audit/final-audit.md — P0 tier 3/3 complete
- `b92b7b8` feat(pmid-resolve): F3 reverse-lookup DOI / PMC → PMID in SmartInput
- `1713fde` docs: mark batch-1 findings DONE in docs/audit/final-audit.md
- `af4fe61` chore: batch-1 quick wins (tracer watchlist, fuzzy pattern, docstring, overfetch cleanup)
- `11b4219` docs: mark F2 DONE in docs/audit/final-audit.md
...
```
M docs/audit/final-audit.md    (F3 status flip + implementation-notes rewrite + handoff
