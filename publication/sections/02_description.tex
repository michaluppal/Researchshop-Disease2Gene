...
\begin{enumerate}[itemsep=2pt]
    ...
    \item \textbf{Citation anchoring.} The citation or evidence quote supplied by the LLM is matched back to the article using normalized dense sequence matching~\cite{python_difflib,ratcliff1988gestalt}. Rows with citation-bearing fields are removed from the primary export when none of the supplied citations can be grounded.
...
These heuristics bound the LLM's role rather than replacing expert curation: the model may summarize and structure evidence, but exported rows must remain tied to identifiers and source passages that ResearchShop can locate in the paper.
...
The thresholds in Table~\ref{tab:llm_validation_decisions} are implementation heuristics selected to favour precision in the default export, not biological constants. The 0.85 citation threshold permits XML, Unicode, and line-wrapping drift while rejecting paraphrases; the $\pm$1,500-character context window allows nearby captions, tables, or lead-in sentences without accepting article-level co-mentions; and the 0.7 confidence threshold is a conservative export cutoff applied only after required hard gates pass, not a calibrated probability.

The remaining validation layers target specific error types:
