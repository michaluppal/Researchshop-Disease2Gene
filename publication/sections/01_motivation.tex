...
In this paper, we present ResearchShop, a text-mining solution for validated gene--disease association extraction from biomedical literature. The tool was born from a scientific necessity: genomic medicine faces a fundamental knowledge-extraction bottleneck in which sequencing data are abundant, but the interpretation of genetic variants relies on evidence scattered across a literature base now measured in tens of millions of records. PubMed indexes more than 40 million citations and abstracts in biomedical and life-science literature~\cite{pubmed_about_2026}. Curated resources such as ClinVar and OMIM provide essential interpretation records built from submitted, reviewed, and periodically updated evidence~\cite{landrum2025clinvar,amberger2015omim}. ResearchShop addresses the upstream literature-triage step: newly published evidence still has to be discovered, read, and assessed before it can inform expert review or later database updates.

This bottleneck has direct consequences. In a cohort of 1,689,845 individuals undergoing hereditary disease genetic testing, 41.0\% had at least one Variant of Uncertain Significance (VUS); among reclassified VUSs, the mean elapsed time from original classification was 22.4 months for upgrades to pathogenic or likely pathogenic and 30.7 months for downgrades to benign or likely benign~\cite{chen2023vus}. The rare-disease setting compounds this problem because diagnosis often depends on combining clinical observations, sequencing results, and dispersed medical literature; the U.S. National Center for Advancing Translational Sciences summarizes 3--15 years as a common rare-disease diagnostic timeline, while genomic reviews emphasize that many patients remain undiagnosed even after exome sequencing~\cite{ncats_rare_diseases_fact_sheet_2023,marwaha2022beyondexome}.
...
\paragraph{LLM hallucinations} Large Language Models show promise for information extraction but are prone to generating plausible yet factually incorrect associations~\cite{farquhar2024semanticentropy}. In this setting, a hallucination is not only an obviously impossible sentence. It can be a biomedical-looking identifier, such as a non-HGNC-approved ``\textit{BRCA3}'' label, or a real gene symbol recalled from model pre-training but absent from the paper being processed. Both failure modes are dangerous because the row may look biologically credible while lacking source-paper support. For biomedical review workflows, therefore, the relevant question is not whether the LLM can produce fluent summaries, but whether each exported association can be tied back to a validated identifier and a recoverable source passage.

Existing tools fall on a spectrum: dedicated NER and biomedical NLP systems such as PubTator Central~\cite{wei2019pubtatorcentral}, BioBERT~\cite{lee2020biobert}, scispaCy~\cite{neumann2019scispacy}, and variant-oriented tools such as tmVar~3.0~\cite{wei2022tmvar3} support entity recognition and normalization, but they do not by themselves provide the schema-driven, auditable extraction workflow needed by researchers reviewing gene--evidence tables. LLM-based approaches offer more flexible contextual extraction, including synthesis of relationships between genes and findings, but require explicit safeguards against unsupported generations~\cite{ji2023hallucination,farquhar2024semanticentropy}. ResearchShop is designed around this separation of responsibilities: NER and deterministic scanning propose source-grounded candidates, the LLM fills a user-defined schema for those candidates, and validation decides what can be exported. Table~\ref{tab:tool_comparison} compares method, full-text access, and hallucination guardrails across these approaches.
...
ResearchShop & PubTator3, deterministic HGNC scanning, and structured LLM extraction & Open-access PMC and Europe PMC full text, including tables and figure captions where available & HGNC validation, source-text grounding, citation/table checks, confidence gating, and audit artifacts \\
\bottomrule
\end{tabularx}
